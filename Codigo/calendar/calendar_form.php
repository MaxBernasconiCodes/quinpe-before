...
$sld = (isset($_REQUEST["sel_day"])) ? $_REQUEST["sel_day"] : 0;
$slm = (isset($_REQUEST["sel_month"])) ? (int)$_REQUEST["sel_month"] : 0;
$sly = (isset($_REQUEST["sel_year"])) ? (int)$_REQUEST["sel_year"] : 0;
...
function setValue(){
	...
	var date_selected = padString(f.sel_day.value, 2, "0") + "-" + padString(f.sel_month.value, 2, "0") + "-" + padString(f.sel_year.value, 4, "0");
...
function selectDay(d){
	closeMe();
	var f = document.calendarform;
	f.sel_day.value = d.toString();
	f.sel_month.value = f.m[f.m.selectedIndex].value;
	f.sel_year.value = f.y[f.y.selectedIndex].value;
	...
	submitNow(f.sel_day.value, f.sel_month.value, f.sel_year.value);
	}
...
<div id="calendar-header" align="center">
	...
	<form id="calendarform" name="calendarform" method="post" action="<?php echo($thispage);?>" style="margin: 0px;">
	  <table border="0" cellspacing="0" cellpadding="1">
		...
		<input name="sel_day" type="hidden" id="sel_day" value="<?php echo($sld);?>" />
		<input name="sel_month" type="hidden" id="sel_month" value="<?php echo($slm);?>" />
		<input name="sel_year" type="hidden" id="sel_year" value="<?php echo($sly);?>" />
...
<div id="calendar-container">
	...
	<div id="calendar-footer">
	  <div class="btn">
		...
		?><a href="javascript:move('<?php echo(str_pad($previous_month, 2, "0", STR_PAD_LEFT));?>', '<?php echo($previous_year);?>');">&lt; Anterior</a><?php
		}else echo("&nbsp;");
		?>
		</div>
		...
		?><a href="javascript:move('<?php echo(str_pad($next_month, 2, "0", STR_PAD_LEFT));?>', '<?php echo($next_year);?>');">Siguiente &gt;</a><?php
		}else echo("&nbsp;");
		?>
		</div>
