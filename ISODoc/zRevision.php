...
if (isset($_POST['CmdAceptar'])){ 
		...
		if ((empty($_POST['TxtFecha1'])) or (empty($_POST['CbPers1']) and empty($_POST['CbProv1']))){
			foreach($_POST as $key => $value){$_SESSION[$key] = $value;}		
			GLO_feedback(3);header("Location:Revision.php?id=".intval($_POST['TxtFR']));
		}else{
			//post
			...
			$ed=0;
			$fv='0000-00-00';
			$fo=intval($_POST['TxtFO']);
			$fr=intval($_POST['TxtFR']);
			...
			$query="INSERT INTO iso_doc (Id,Codigo,Nombre,Version,IdSector,IdTipoDoc,IdNorma,IdReq,Origen,Ruta,FechaCRE,FechaCON,FechaAPR,FechaEXP,IdPersCRE,IdProvCRE,IdPersCON,IdPersAPR,ComentCRE,ComentCON,ComentAPR,IdEstado,FlagOrig,FlagRev,Editable) VALUES ($nroId,'$cod','$nom',$vs,$sec,$tipo,$nor,$req,$ori,'','$f1','$fv','$fv','$fv',$res1,$res1pr,$res2,$res3,'$com1','','',0,$fo,$fr,$ed)"; 
			//echo $query;
			$rs=mysql_query($query,$conn);
			if ($rs){	GLO_feedback(1);$grabook=1;	ISODOC_grabaauditoria($nroId,7,$conn);}else{GLO_feedback(2);$grabook=0; } 
			mysql_close($conn); 
			//volver
			if($grabook==1){
				...
				foreach($_POST as $key => $value){$_SESSION[$key] = $value;}
				header("Location:Revision.php?id=".intval($_POST['TxtFR']));
			}		
		}
}
