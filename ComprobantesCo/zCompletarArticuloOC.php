...
elseif (isset($_POST[CmdSalir])){
	...
	header("Location:AltaItemOC.php?Id=".intval($_POST['TxtNroEntidad']));
...
elseif (isset($_POST['CmdExBuscar']) or isset($_POST['CmdExBuscar2'])){
	...
	header("Location:CompletarArticuloOC.php?Id=".intval($_POST['TxtNroEntidad'])."&IdItem=".intval($_POST['TxtNumero']));
}
...
elseif (isset($_POST[CmdAceptar])){
	if(intval($_POST['CbItem'])!=0 and intval($_POST['CbItem2'])!=0){		
		...
		$query="UPDATE co_npedido_it set IdArticulo=$iditem,IdItem=$iditem2 Where INC=1 and Id=".intval($_POST['TxtNumero']);
