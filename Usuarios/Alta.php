...
function ComboNombreUsuario($conn){ 
	$query="SELECT Id,Nombre,Apellido FROM personal where Id<>0 and FechaBaja='0000-00-00' Order by Apellido,Nombre";$rs=mysql_query($query,$conn);
	$combo="";
	while($row=mysql_fetch_array($rs)){ 
	  if( $row['Id'] == $_SESSION['CbNombre']) {$combo .= " <option value='".$row['Id']."' selected='"."selected". "'>".substr($row['Apellido']." ".$row['Nombre'],0,25)."</option>\n";
	   }else{$combo .= " <option value='".$row['Id']."'>".substr($row['Apellido']." ".$row['Nombre'],0,25)."</option>\n"; }
...
GLOF_Init('CbNombre','BannerConMenuHV','zAlta',0,'MenuH',0,0,0); 
GLO_tituloypath(0,600,'../Usuarios.php','USUARIOS','linksalir'); 
?> 


<table width="600" border="0"   cellspacing="0" class="Tabla" >
<tr> <td width="100" height="5"  ></td> <td width="500"></td> </tr>
<tr><td height="18"  align="right"  >Puesto:</td><td valign="top" >&nbsp;<select name="CbPuesto" class="campos" id="CbPuesto" style="width:230px" ><? USR_ComboPuesto(1);?> </select> <label class="MuestraError"> * </label></td></tr>
...
<tr><td height="18"  align="right"  >Perfil:</td><td  valign="top" >&nbsp;<select name="CbPerfil"  class="campos" id="CbPerfil" style="width:230px"><option value=""></option> <? USR_ComboPerfil(1,$conn); ?> </select> <label class="MuestraError"> * </label></td></tr>
</table>
