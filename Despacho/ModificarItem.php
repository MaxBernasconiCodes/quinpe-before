...
if ($_GET['Flag1']=="True"){
	$query="SELECT m.*,a.IdCliente,a.IdTipo,a.IdPadre as IdSoli From despacho_it m,despacho a where m.IdPadre=a.Id and m.Id=".intval($_GET['id']);$rs=mysql_query($query,$conn);$row=mysql_fetch_array($rs);
...
//despacho solo modifica si esta pendiente
if(intval($_SESSION['TxtLocPage'])==0 and intval($_SESSION['CbEstado'])<2){GLO_guardar(730,3,0);}
//planta solo modifica si esta aceptado planta
if(intval($_SESSION['TxtLocPage'])==1 and intval($_SESSION['CbEstado'])==2){GLO_guardar(730,3,0);}
