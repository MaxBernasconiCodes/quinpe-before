...
function MostrarTabla($conn){
	$query=$_SESSION['TxtQACCCERT'];$query=str_replace("\\", "", $query);
	if (  ($query!="")){
		...
		if(mysql_num_rows($rs)!=0){	
			...
			$tablaclientes .="<td "."width="."280"." class="."TableShowT"."> Accesorio</td>";   
			$tablaclientes .="<td "."width="."160"." class="."TableShowT"."> Elemento</td>";   
			...
			while($row=mysql_fetch_array($rs)){
				...
				$tablaclientes .='<td class="'.$clase.'"'.$link."> ".substr($row['Ins'],0,20)."</td>"; 
...
GLO_tituloypath(0,760,'Consulta.php','CERTIFICACIONES ACCESORIOS','linksalir');

?>

<table width="760" border="0"  cellspacing="0" class="Tabla" >
<tr> <td height="5" width="80"></td><td width="100"></td><td width="130"></td><td width="80"></td><td width="100"></td><td width="50"></td><td width="100"></td><td width="100"></td></tr>
<tr> <td height="18"  align="right">&nbsp;Fecha:</td><td  >&nbsp;<? GLO_calendario("TxtFechaD","../Codigo/","actual",1) ?></td><td  > al&nbsp;<? GLO_calendario("TxtFechaH","../Codigo/","actual",1) ?></td><td align="right">Elemento:</td><td  >&nbsp;<select name="CbInstrumento" class="campos" id="CbInstrumento"  style="width:80px" onKeyDown="enterxtab(event)"><option value=""></option><? GLO_ComboEquipos("CbInstrumento","accesorios",$conn); ?></select></td><td align="right">Certif:</td><td colspan="2">&nbsp;<select name="CbCertif" class="campos" id="CbCertif"  style="width:80px" onKeyDown="enterxtab(event)"><option value=""></option><? ComboTablaRFX("instrumentostipocertif","CbCertif","Nombre","","",$conn); ?></select></td></tr>

<tr> <td height="18"  align="right">Vencimiento:</td><td  >&nbsp;<? GLO_calendario("TxtFechaDCP","../Codigo/","actual",1) ?></td><td  > al&nbsp;<? GLO_calendario("TxtFechaHCP","../Codigo/","actual",1) ?></td><td  align="right">Tipo:</td><td>&nbsp;<select name="CbTInstrumento" class="campos" id="CbTInstrumento"  style="width:80px" onKeyDown="enterxtab(event)"><option value=""></option><? ComboTablaRFX("accesorios_tipo","CbTInstrumento","Nombre","","",$conn); ?></select></td><td></td><td><input name="ChkVtoCP"  type="checkbox"  value="1" <? if ($_SESSION['ChkVtoCP'] =='1') echo 'checked'; ?>>Vencidos</td><td   align="right" ><? GLO_Search('CmdBuscar',0); ?>&nbsp;</td></tr>
</table>
...
<? 
GLO_Hidden('TxtId',0);GLO_Hidden('TxtQACCCERT',0);
GLO_linkbutton(760,'Agregar','AltaCE.php','','','','');
GLO_mensajeerror();
MostrarTabla($conn);
GLO_cierratablaform();
mysql_close($conn);
