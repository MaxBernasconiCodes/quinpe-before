...
elseif (isset($_POST['CmdGuardar'])){	 
	$idcompr=intval($_POST['TxtNroEntidad']);
	 if(!empty($_POST['campos'])) {
		...
		foreach($aListaid as $iId) {
			...
			$cant=floatval($aListacant[$iIdItemNP]);	
	...
	$_SESSION['TxtQSTRIIT']=$_POST['TxtQSTRIIT'];
	header("Location:AltaItemRI.php?Id=".intval($_POST['TxtNroEntidad']));
}
