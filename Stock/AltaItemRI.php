...
function MostrarTabla($conn){
	$query=$_SESSION['TxtQSTRIIT'];$query=str_replace("\\", "", $query); 
	if ( $query!=""){	
		...
		if(mysql_num_rows($rs)!=0){
			...
			<td width="70" class="TableShowT"> OC</td>   
			...
			<td width="100" class="TableShowT"> Proveedor</td> 
			...
			while($row=mysql_fetch_array($rs)){ 
				...
				<td class="TableShowD">'.$row['NroOC'].'</td> 
				...
				<td class="TableShowD">'.substr($row['Prov'],0,12).'</td>  
...
GLO_InitHTML($_SESSION["NivelArbol"],'','BannerPopUpMH','zAltaItemRI',0,0,0,0);
GLO_tituloypath(0,700,"Modificar.php?id=".intval($_SESSION['TxtNroEntidad'])."&Flag1=True",'AGREGAR ITEMS OC A MOV INGRESO','linksalir');
include("Includes/zFiltrosItems.php");
GLO_Hidden('TxtNroEntidad',0);GLO_Hidden('TxtQSTRIIT',0);
GLO_mensajeerror();
MostrarTabla($conn);
GLO_cierratablaform();
mysql_close($conn); 
...
echo 'Solo mostrar&aacute; <font class="comentario3">Items de OC </font> que no se hayan asociado a ning&uacute;n Remito o Ajuste <font class="comentario2">Ingreso</font><br>';
echo 'Para asociar Items al movimiento y marcarlos como <font class="comentario2">Comprados</font> seleccione el boton <i class="fa fa-save iconvsmallsp iconlgray"></i> <font class="comentario3">Agregar</font> <br>';
GLO_endcomment();
