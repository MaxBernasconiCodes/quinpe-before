...
function MostrarTabla($conn){
	$query=$_SESSION['TxtQSTCOMPRAR'];$query=str_replace("\\", "", $query); 
	if ( $query!=""){	
		...
		if(mysql_num_rows($rs)!=0){
			...
			//guardar		
			echo '<table width="1130"><tr><td class="comentario">Seleccione el Item y luego grabe</td><td align="right">'.GLO_FAButton('CmdComprar','submit','90','self','Comprar','cart','boton02').'</td></tr></table>';
			//Titulos de la tabla
			...
			<td width="40" class="TableShowT"> </td> 
			...
			<td width="100" class="TableShowT">Dividir Item</td>
			</tr>';             
			$recuento=0;  
			while($row=mysql_fetch_array($rs)){ 
				...
				//solo adjunta/compra si tiene articulo/prod
				if($row['IdArticuloItem']>0 or $row['IdProd']>0){
					echo '<input type="checkbox" name="campos['.$row['IdItemNP'].']" unchecked value=0 onclick="if (this.checked==1) {this.value=1;}else{this.value=0;}" onChange="CheckRow('.$row['IdItemNP'].',this.value);">';
				}else{
					//si no tiene articulo completa	
					echo GLO_rowbutton("CmdCompletar",$row['IdItemNP'],"Completar",'self','add','iconlgray','Completar',0,0,0); 
				}
				echo '</td><td class="TableShowD TAC">';
				/*				
				//solo modifica si completo articul oporque estaba incompleto y lo debe modificar
...
GLOF_Init('','BannerConMenuHV','zComprar',0,'MenuH',0,0,0); 
GLO_tituloypath(0,700,"../Stock.php",'REVISAR ITEMS A COMPRAR','linksalir');
?>
<table width="700" border="0"  cellspacing="0" class="Tabla" >
<tr> <td height="5" width="70"></td><td width="100"></td><td width="120"></td><td width="80"></td><td width="120"></td><td width="80"></td><td width="90"></td><td width="30"></td></tr>
...
<?
GLO_Hidden('TxtQSTCOMPRAR',0);GLO_Hidden('TxtId',0);
GLO_mensajeerror();
MostrarTabla($conn);
GLO_cierratablaform();
mysql_close($conn); 
