...
$id=intval($_GET['Id']);//articulo
$iddep=intval($_GET['IdD']);//deposito
$idcliprop=intval($_GET['IdP']);//propietario
$query="SELECT s.*,d.Nombre as Deposito,t.Nombre as TipoM,a.Nombre as Articulo,i.Cantidad,i.IdArticulo,i.Id as IdIST,c.Nombre as Cliente From stockmov s,depositos d,stock_tipomov t,stockmov_items i,epparticulos a,clientes c where s.IdDeposito=d.Id and s.IdTipoMov=t.Id and i.IdMov=s.Id and i.IdArticulo=a.Id and s.IdCliente=c.Id  and i.IdArticulo=$id and s.IdDeposito=$iddep and s.IdCliente=$idcliprop Order by s.Fecha,s.Id";
