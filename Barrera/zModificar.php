<? include("../Codigo/Seguridad.php");include("../Codigo/Config.php");include("../Codigo/Funciones.php");include("Includes/zFuncionesCheck.php");
//perfiles
GLO_PerfilAcceso(13);
...
elseif (isset($_POST['CmdBorrarSoli'])){
	...
	if ($rs){
		GLO_feedback(1);
		//borro proceso
		$query="Delete From procesosop Where Id<>0 and Id=".intval($_POST['TxtNroEntidad']);$rs=mysql_query($query,$conn);	
	}else{GLO_feedback(2);} 
	mysql_close($conn); 
	header("Location:Modificar.php?id=".intval($_POST['TxtNumero'])."&Flag1=True");	
}
...
elseif (isset($_POST['CmdBuscarD1'])){
	...
	if($dom!=''){
		...
		$query="SELECT * FROM procesosop_e1 Where Dominio='$dom' and Dominio<>'' Order by Id desc LIMIT 1";$rs=mysql_query($query,$conn);
		$row=mysql_fetch_array($rs);if(mysql_num_rows($rs)!=0){
