...
if (isset($_POST['CmdAceptar'])){
	...
	$error=ASIG_validarquien(intval($_POST['CbInstrumento']),intval($_POST['CbPersonal']),intval($_POST['CbUnidad']),intval($_POST['CbSector']));
	if ( $error==1 ){
		...
		if($fechasok==1){	
			//inserto 
			$nroId=GLO_generoID('instrumentosasig',$conn); 
			$query="INSERT INTO instrumentosasig (Id,IdInstrumento,FechaD,FechaH,Obs,IdPersonal,IdUnidad,IdSectorM,TIndef,FechaE) VALUES ($nroId,$idinstr,'$fechaa','0000-00-00','$obs',$pers,$uni,$sec,$ti,'$fechae')";	$rs=mysql_query($query,$conn);		
