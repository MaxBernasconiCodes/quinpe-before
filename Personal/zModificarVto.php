...
if (isset($_POST['CmdAceptar'])){
	if ((empty($_POST['CbTipo'])) or (empty($_POST['TxtFechaA'])) ){
		...
		//actualizo
		$query="UPDATE personalvtos set IdTipo=$tipo,FechaE='$fechaa',Fecha='$fechab',Req=$req,Obs='$obs',Inactivo=$inac Where Id=".intval($_POST['TxtNumero']);
...
elseif (isset($_POST['CmdVerArchivoH'])){
	GLO_OpenFile("personalvtos",intval($_POST['TxtNumero']),"Adjuntos/","Certif");
}
elseif (isset($_POST['CmdBorrarArchivoH'])){
	$conn=mysql_connect($server,$dbuser,$dbpass);mysql_select_db($database);
	$query="UPDATE personalvtos set Certif='' Where Id=".intval($_POST['TxtNumero']);
	$rs=mysql_query($query,$conn);
	if ($rs){GLO_feedback(1);}else{GLO_feedback(2); } 
	mysql_close($conn); 
	//limpiar datos del form anterior
...
elseif (isset($_POST['CmdBorrarFilaA'])){
	...
	//busco path
	$query="SELECT Ruta From personalvtos_a Where Id=$id";$rs=mysql_query($query,$conn);$row=mysql_fetch_array($rs);
	if(mysql_num_rows($rs)!=0){$archivo=$row['Ruta'];}else{$archivo="";}mysql_free_result($rs);
	//elimino
	$query="Delete From personalvtos_a Where Id=$id";$rs=mysql_query($query,$conn);	
	if($rs){unlink('../Archivos/Adjuntos/'.$archivo) ;} 
	mysql_close($conn); 	
	header("Location:ModificarVto.php?id=".intval($_POST['TxtNumero'])."&Flag1=True");
}
elseif (isset($_POST['CmdVerFile'])){
	GLO_OpenFile("personalvtos_a",intval($_POST['TxtId']),"Adjuntos/","Ruta");
}
