...
if (isset($_POST['CmdAceptar'])){ 
	if ((empty($_POST['TxtFechaA']))  or (empty($_POST['TxtOrg'])) ){
		...
		GLO_feedback(3);header("Location:AltaAntec.php?Id=".intval($_POST['TxtNroEntidad']));
	}else{
		...
		//inserto
		$nroId=GLO_generoID('personal_antec',$conn);
		$query="INSERT INTO personal_antec (Id,IdEntidad,FechaD,FechaH,Org,Obs) VALUES ($nroId,$ident,'$fechaa','$fechab','$org','$obs')";$rs=mysql_query($query,$conn);
		if ($rs){GLO_feedback(1);$grabook=1;}else{GLO_feedback(2);$grabook=0; } 
		mysql_close($conn); 	
		//volver
		if($grabook==1){
			foreach($_POST as $key => $value){$_SESSION[$key] = "";}
			header("Location:AltaAntec.php?Id=".intval($_POST['TxtNroEntidad'])."&Flag1=True");
		}else{
			foreach($_POST as $key => $value){$_SESSION[$key] = $value;}
			header("Location:AltaAntec.php?Id=".intval($_POST['TxtNroEntidad']));
		}			
	}
}
