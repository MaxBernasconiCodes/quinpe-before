...
$query=$_SESSION['TxtQueryRepTar'];$query=str_replace("\\", "", $query);
if (  ($query!="")){	
	$rs=mysql_query($query,$conn);
	if(mysql_num_rows($rs)!=0){	
		//Titulos de la tabla
		$tablaclientes='';
		$tablaclientes .=GLO_inittabla(1040,1,0,0);
		...
		$tablaclientes .="<td "."width="."50"." class="."TableShowT"."> H.Inicio</td>";   
		$tablaclientes .="<td "."width="."50"." class="."TableShowT"."> H.Fin</td>";   
		...
		while($row=mysql_fetch_array($rs)){ 			
			...
			$tablaclientes .="<td class=".$clase.$link."> ".GLO_FormatoHora($row['Hora1'])."</td>"; 
			$tablaclientes .="<td class=".$clase.$link."> ".GLO_FormatoHora($row['Hora2'])."</td>"; 
			$tablaclientes .="<td class=".$clase.$link."> ".substr($row['ObsT'],0,36)."</td>"; 
			$tablaclientes .='</tr>'; 
			$recuento=$recuento+1;
...
GLOF_Init('TxtNroInterno','BannerConMenuHV','zTareas',0,'MenuH',0,0,0); 
GLO_tituloypath(0,750,'Reportes.php','TAREAS','linksalir');


include("Includes/zFiltrosRep.php") ;
GLO_Hidden('TxtQueryRepTar',0);
GLO_mensajeerror();
MostrarTabla($conn); 
GLO_cierratablaform();
mysql_close($conn);
