...
if (isset($_POST['CmdAceptar'])){ 
	...
	if ( (empty($_POST['TxtObs'])) ){
		...
		GLO_feedback(3);header("Location:AltaOrdenDSoliR.php?id=".intval($_POST['CbSoli']));
		...
		if($idorden==0){//verifica que no tenga orden asociada
			...
			$query="INSERT INTO pedidosrepord (Id,IdUnidad,IdSector,IdInstr,Fecha,FechaI,FechaE,FechaIT,IdEstado,Km,Hs,Obs,IdSoli,IdPersonalPL,FechaPL,ObsPL,ListoPL".$wq1.") VALUES ($nroId,$uni,$sec,$ins,'$fecha1','0000-00-00','0000-00-00','0000-00-00',1,0,0,'$obs',$soli,0,'0000-00-00','',0".$wq2.")"; 

			$rs=mysql_query($query,$conn); 

			if ($rs){

				$grabook=1;REP_updateestadoorden($conn,$nroId,7);

				//asigo orden a la solicitud

				if ($soli!=0){

					$query="UPDATE pedidosrep set IdOrden=$nroId,IdEstado=2 Where IdOrden=0 and Id=$soli";$rs=mysql_query($query,$conn);
		...
		if($grabook==1){
			...
			header("Location:AltaOrdenDSoliR.php?id=".intval($_POST['CbSoli']));//pasa el nro de solicitud
