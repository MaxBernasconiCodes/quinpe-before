...
$query="select u.*,c.Nombre,c.Apellido,p.Nombre AS NombrePerfil from usuarios u,personal c,perfiles p where u.IdPersonal= c.Id and u.IdPerfil = p.Id and  u.Tipo='PERSONAL' and u.Usuario<>'admin' Order By u.FechaBaja,c.Apellido,c.Nombre";$rs=mysql_query($query,$conn);
...
while($row=mysql_fetch_array($rs)){
	$link=" onclick="."location='Usuarios/Modificar.php?id=".$row['Usuario']."'";
	if($row['IdPerfil']==2){$claseperfil=' TRed';}else{$claseperfil='';}
	$fbaja= GLO_FormatoFecha($row['FechaBaja']);if ($fbaja==''){$clase="";}else{$clase=" TGray";$claseperfil='';}	
	//
	...
	$tablaclientes .='<td class="TableShowD'.$clase.$claseperfil.'"'.$link."> ".substr($row['NombrePerfil'],0,25)."</td>";  
	$tablaclientes .='<td class="TableShowD'.$clase.'"'.$link."> ".$fbaja."</td>";  
	$tablaclientes .='<td class="TableShowD TAC">';
	if ($fbaja==''){
		$tablaclientes .='<a href="Usuarios/CambioClave.php?id='.$row['Usuario'].'" title="Clave"><i class="fa fa-key iconsmallbt iconlgray"></i></a>'; 
	}
	$tablaclientes .='</td></tr>'; 
	$recuento=$recuento+1;
...
GLO_tituloypath(0,750,'Inicio.php','USUARIOS PERSONAL','linksalir'); 
?>

<table  width="750" border="0" cellspacing="0" cellpadding="0" >
<tr><td  height=3 ></td></tr>
<tr><td align="right" valign="bottom" >
<input name="CmdAgregar" type="button" class="boton" value="Agregar" onClick="document.Formulario.target='_self';window.location.href='Usuarios/Alta.php'">
</td></tr></table>
