...
if (isset($_POST['CmdBuscar'])){ 	
	...
	$vbuscar=intval($_POST['TxtIdOp']);if($vbuscar!=0){$wbuscar=$wbuscar." and a.IdOp=$vbuscar";}	
	//
	$_SESSION['TxtQCOTV']="Select a.*,c.Nombre,e.Nombre as Estado,s1.Nombre as LA From c_cotizaciones a,clientes c,c_coti_estados e,serviciostipo1 s1 Where a.IdCliente=c.Id and e.Id=a.IdEstado and s1.Id=a.IdTipo $wbuscar Order by a.Id";
...
elseif (isset($_POST['CmdBorrarFila'])){
	...
	//valido si tiene oportunidad
	$query="SELECT IdOp From c_cotizaciones Where Id=".intval($_POST['TxtId']); $rs2=mysql_query($query,$conn);$row2=mysql_fetch_array($rs2);if(mysql_num_rows($rs2)!=0){$idop=$row2['IdOp'];}else{$idop=0;}mysql_free_result($rs2);	
	//elimino cotizacion
	$query="DELETE From c_cotizaciones Where Id=".intval($_POST['TxtId']);$rs=mysql_query($query,$conn);
	if ($rs){
		GLO_feedback(1);
		//cambio estado de oportunidad a aceptada (si tiene)
		if($idop!=0){$query="UPDATE c_oportunidades set IdEstado=1 Where Id=$idop";$rs=mysql_query($query,$conn);}	
	}else{GLO_feedback(2);} 
	mysql_close($conn); 
	header("Location:Cotizaciones.php"); 	
}
...
elseif (isset($_POST['CmdExcel'])){
	$query=$_POST['TxtQCOTV'];$query=str_replace("\\", "", $query);
	if ($query!=""){
