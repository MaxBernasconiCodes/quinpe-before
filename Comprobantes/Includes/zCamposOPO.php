...
GLO_tituloypath(0,720,'Oportunidades.php','OPORTUNIDAD','linksalir');
?>
...
<tr> <td height="18"  align="right"  >Cliente:</td><td  valign="top">&nbsp;<select name="CbCliente" style="width:220px" class="campos" tabindex="1" id="CbCliente" > <? if(intval($_SESSION['TxtNumero'])==0){echo '<option value=""></option>';GLO_ComboActivo("clientes","CbCliente","Nombre","","",$conn);}else{ComboTablaRFROX("clientes","CbCliente","Nombre","",$_SESSION['CbCliente'],"",$conn);} ?> </select><label class="MuestraError"> * </label></td><td height="18"  align="right"  >Estado:</td><td  valign="top" colspan="2">&nbsp;<select name="CbEstado" style="width:135px" class="campos" tabindex="1" id="CbEstado" > <? if(intval($_SESSION['CbEstado'])!=3){echo '<option value=""></option>';ComboTablaRFX("c_oportunidad_est","CbEstado","Nombre","","and Id IN(1,2)",$conn);}else{ComboTablaRFROX("c_oportunidad_est","CbEstado","Nombre","",$_SESSION['CbEstado'],"",$conn);} ?> </select> </td></tr>

<tr> <td height="18"  align="right"  >Vendedor:</td><td  valign="top">&nbsp;<select name="CbPersonal" class="campos" id="CbPersonal"  tabindex="1" style="width:220px" onKeyDown="enterxtab(event)"><option value=""></option><? ComboPersonalRFX('CbPersonal',$conn); ?></select><label class="MuestraError"> * </label></td><td  align="right"  >Cotizacion:</td><td colspan="2">&nbsp;<input name="TxtIdCot" type="text"  class="TextBoxRO" style="text-align:right;width:65px" readonly="true" value="<? echo $_SESSION['TxtIdCot']; ?>"> <? if(intval($_SESSION['CbEstado'])!=3 and intval($_SESSION['TxtNumero'])!=0){ echo '<input name="CmdCotizar" type="submit" class="boton03" style="height:18px;width:65px"  value="Cotizar" onClick="document.Formulario.target='."'_self'".'">';}?></td></tr>

<tr> <td height="18"  align="right"  >Tipo Servicio:</td><td  valign="top">&nbsp;<select name="CbTipo"  style="width:220px" class="campos" tabindex="1"  id="CbTipo" ><option value=""></option><? ComboTablaRFX("serviciostipo1","CbTipo","Nombre","","",$conn); ?> </select></td><td  align="right"  ></td><td colspan="2">&nbsp;</td></tr>
...
<? 
GLO_Hidden('TxtId',0);
//si no esta cotizada graba
if(intval($_SESSION['CbEstado'])!=3){GLO_guardar("720",2,0);}
GLO_mensajeerror();
?>